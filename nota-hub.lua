...
-- Переменные состояний по умолчанию
local isCollectEnabled = true
local isBagEnabled = true
local isFarmEnabled = true
local isUpgradeEnabled = true
local isRebirthEnabled = false
local isBossEnabled = true
local isBossFighting = false
local targetSpeed = 60
local isNoclipEnabled = false
local isFlyEnabled = false

-- Чтение конфига с диска при старте
local savedConfig = {}
pcall(function()
    if readfile and isfile and isfile("NotaHubConfigs/NotaHubConfig.json") then
        local content = readfile("NotaHubConfigs/NotaHubConfig.json")
        savedConfig = HttpService:JSONDecode(content) or {}
    end
end)

-- Сразу прокидываем значения переменных из сохраненного конфига
if savedConfig["AutoCollectFlag"] ~= nil then isCollectEnabled = savedConfig["AutoCollectFlag"] end
if savedConfig["AutoBagFlag"] ~= nil then isBagEnabled = savedConfig["AutoBagFlag"] end
if savedConfig["AutoFarmFlag"] ~= nil then isFarmEnabled = savedConfig["AutoFarmFlag"] end
if savedConfig["AutoUpgradeFlag"] ~= nil then isUpgradeEnabled = savedConfig["AutoUpgradeFlag"] end
if savedConfig["AutoRebirthFlag"] ~= nil then isRebirthEnabled = savedConfig["AutoRebirthFlag"] end
if savedConfig["NoclipFlag"] ~= nil then isNoclipEnabled = savedConfig["NoclipFlag"] end
if savedConfig["FlyFlag"] ~= nil then isFlyEnabled = savedConfig["FlyFlag"] end
if savedConfig["AutoBossFlag"] ~= nil then isBossEnabled = savedConfig["AutoBossFlag"] end
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "by @nado00010",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = "NotaHubConfigs",
        FileName = "NotaHubConfig",
        SaveInRoot = true
    },
    KeySystem = false, 
})
...
task.spawn(function()
    ...
    while true do
        if isFarmEnabled and not isBossFighting then
            ...
            if os.clock() - lastGoBackClick >= 4.8 then
                clickActiveGoBack()
...
-- Создание элементов интерфейса с привязкой к уже загруженным переменным
TabFarm:CreateToggle({
    Name = "Auto Collect",
    CurrentValue = isCollectEnabled,
